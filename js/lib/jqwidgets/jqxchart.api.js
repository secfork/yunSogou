...
(function (a) {
    a.extend(a.jqx._jqxChart.prototype, {_moduleApi: true, getItemsCount: function (f, b) {
        ...
        for (var d = 0; d < j.items.length; d++) {
            if (e) {
                c.push({offset: {x: j.itemOffsets[j.items[d]].x + j.itemWidth / 2, y: j.rect.y + (b ? j.rect.height : 0)}, value: j.items[d]})
            } else {
                c.push({offset: {x: j.rect.x + j.rect.width, y: j.itemOffsets[j.items[d]].y + j.itemWidth / 2}, value: j.items[d]})
            }
        }
        return c
