...
(function (a) {
    a.extend(a.jqx._jqxChart.prototype, {_moduleRangeSelector: true, _renderXAxisRangeSelector: function (o, m) {
        var q = this;
        ...
        c -= 4;
        var n = this._getXAxisStats(o, d, c);
        var j = d.position;
        ...
        if (!this._isSelectorRefresh) {
            ...
            if (!l) {
                ...
                e.css({width: l.width(), height: l.height()});
                m.width = l.width();
                m.height = l.height()
            }
        ...
        if (n == undefined && !u.renderTo) {
            n = {left: q, top: y, right: z, bottom: C}
        } else {
            n = {left: ((n && n.left) ? n.left : q), top: ((n && n.top) ? n.top : y), right: ((n && n.right) ? n.right : z), bottom: ((n && n.bottom) ? n.bottom : C)}
        }
        ...
        o.addHandler(a(document), o._getEvent("mousemove") + "." + o.element.id, o._onSliderMouseMove, {self: this, groupIndex: f, renderTo: A, swapXY: b});
        o.addHandler(a(A), o._getEvent("mousedown"), o._onSliderMouseDown, {self: this, groupIndex: f, renderTo: A, swapXY: b});
        o.addHandler(a(A), o._getEvent("mouseup"), o._onSliderMouseUp, {self: this, groupIndex: f, renderTo: A, swapXY: b})
        ...
        b.css({background: q, opacity: 0.1, left: m.x, top: m.y, width: m.width, height: m.height});
        b.appendTo(l);
        if (!this._sliders) {
            this._sliders = []
        }
        ...
        this._sliders[r] = {element: b, host: l, fullRect: {x: b.coord().left, y: b.coord().top, width: m.width, height: m.height}, rect: m, left: a(j), right: a(j), leftTop: a(c), rightTop: a(c), leftBorder: a(c), leftBar: a(h), rightBorder: a(c), rightBar: a(h)};
        ...
        if (this.seriesGroups[r].orientation != "horizontal") {
            b.css({left: Math.round(m.x + i), top: m.y, width: Math.round(k - i), height: m.height})
        } else {
            b.css({top: Math.round(m.y + i), left: m.x, height: Math.round(k - i), width: m.width})
        }
        ...
        n.rightBar.css(i, 5);
        n.rightBar.css(p, s)
    }, _resizeState: {}, _onSliderMouseDown: function (d) {
        var b = d.data.self;
        ...
        j._raiseEvent("rangeSelectionChanging", {instance: j, minValue: h, maxValue: p});
        j.enableAnimations = false;
        j.update();
        j.enableAnimations = s;
        j._raiseEvent("rangeSelectionChanged", {instance: j, minValue: h, maxValue: p})
        ...
        if (f[j] >= h[j] && f[j] <= h[j] + m[i]) {
            if (Math.abs(f[p] - h[p]) <= n) {
                ...
                q._resizeState = {state: "ready", side: "left"}
            } else {
                if (Math.abs(f[p] - h[p] - (!g ? e.width() : e.height())) <= n) {
                    k.css("cursor", g ? "row-resize" : "col-resize");
                    q._resizeState = {state: "ready", side: "right"}
                } else {
                    if (f[p] + n > h[p] && f[p] - n < h[p] + (!g ? e.width() : e.height())) {
                        k.css("cursor", "pointer");
                        q._resizeState = {state: "ready", side: "move", startPos: f[p]}
                    } else {
