...
(function (d) {
    var b = {defineInstance: function () {
        ...
        var g = this._originalRadius, e = this.renderer._toLinearGradient("#ffffff", [
            [3, 2],
            [100, 1]
        ], true), f = this._outerBorderOffset();
        ...
        var n = this;
        var p = this.ticksMinor, e = this.ticksMajor, l, o, m = {};

        function k(i) {
            n._drawTick({angle: n._getAngleByValue(i), distance: n._getDistance(n.ticksDistance), style: e.style, size: n._getSize(e.size), type: "major"})
        }

        function f(j) {
            n._drawTick({angle: n._getAngleByValue(j), distance: n._getDistance(n.ticksDistance), style: p.style, size: n._getSize(p.size), type: "minor"})
        }

        if (n.int64 === true) {
            l = new d.jqx.math().fromString((p.interval).toString(), 10);
            o = new d.jqx.math().fromString((e.interval).toString(), 10);
            for (var h = new d.jqx.math().fromString((n.min).toString(), 10), g = new d.jqx.math().fromString((n.min).toString(), 10); h.lessThanOrEqual(n._max64) || g.lessThanOrEqual(n._max64); h = h.add(l), g = g.add(o)) {
                if (g.lessThan(n._min64) || g.greaterThan(n._max64)) {
                    break
                }
                if (g.lessThanOrEqual(n._max64) && e.visible) {
                    k(g);
                    m[g.toString()] = true
                }
                if (!m[h.toString()] && p.visible && h.lessThanOrEqual(n._max64)) {
                    f(h)
                }
            }
        } else {
            l = p.interval;
            o = e.interval;
            for (var h = this.min, g = this.min; h <= this.max || g <= this.max; h += l, g += o) {
                if (g <= this.max && e.visible) {
                    k(g);
                    m[g.toFixed(5)] = true
                }
                if (!m[h.toFixed(5)] && p.visible && h <= this.max) {
                    f(h)
                }
            }
        }
        ...
        var n = this.that;
        if ((n.int64 === true && (m._startValue64.lessThan(n._min64) || m._endValue64.greaterThan(n._max64))) || (n.int64 === false && (m.startValue < n.min || m.endValue > n.max))) {
            return
        }
        var q = n.int64 ? n._getAngleByValue(m._startValue64) : n._getAngleByValue(m.startValue), j = n.int64 ? n._getAngleByValue(m._endValue64) : n._getAngleByValue(m.endValue);
        var o = n._originalRadius, f = o - n._getRangeDistance(m.startDistance, m.startWidth), s = o - n._getRangeDistance(m.endDistance, m.endWidth), l = m.startWidth, e = m.endWidth, k = n._outerBorderOffset(), i = {x: o + k + f * Math.sin(q), y: o + k + f * Math.cos(q)}, r = {x: o + k + s * Math.sin(j), y: o + k + s * Math.cos(j)}, t = n._getProjectionPoint(q, o + k, f, l), p = n._getProjectionPoint(j, o + k, s, e), h = "default", u, m;
        ...
        return e
    }, _addLabels: function (g) {
        var f = this;
        if (f.labels.visible) {
            var i = this._getDistance(this._getLabelsDistance()), h;
            if (f.int64 === true) {
                for (var e = new d.jqx.math().fromNumber(f.min.toString(), 10); e.lessThanOrEqual(f._max64); e = e.add(f.labels._interval64)) {
                    if (e.lessThan(f._min64) || e.greaterThan(f._max64)) {
                        break
                    }
                    this._addLabel({angle: this._getAngleByValue(e), value: e.toString(), distance: i, style: this.labels.className})
                }
            } else {
                for (var e = this.min; e <= this.max; e += this.labels.interval) {
                    this._addLabel({angle: this._getAngleByValue(e), value: this.labels.interval >= 1 ? e : new Number(e).toFixed(2), distance: i, style: this.labels.className})
                }
            }
        }
        ...
        var f = this;
        if ((f.int64 === true && h.lessThanOrEqual(f._max64) && h.greaterThanOrEqual(f._min64)) || (f.int64 === false && h <= f.max && h >= f.min)) {
    ...
    var c = {_events: ["valueChanging", "valueChanged"], _animationTimeout: 10, _schemes: [
        ...
        m = {size: this._getSize(l.size), style: l.style, visible: l.visible, interval: i};
        j = {size: this._getSize(k.size), style: k.style, visible: k.visible, interval: h, checkOverlap: true};
    ...
    }, _drawTicks: function (j, h, l) {
        var g = this, e;

        function f(m) {
            e = g._valueToCoordinates(m);
            if (!j.checkOverlap || !g._overlapTick(m)) {
                g._renderTick(j.size, e, j.style, l)
            }
        }

        if (this.int64 === true) {
            for (var k = new d.jqx.math().fromString(this._min64.toString(), 10); k.lessThanOrEqual(this._max64); k = k.add(j.interval)) {
                f(k)
            }
        } else {
            for (var k = this.min; k <= this.max; k += j.interval) {
                f(k)
            }
        }
        ...
        if (this.int64 === true) {
            e = e.add(this._min64);
            if ((e.modulo(this.ticksMinor._interval64)).equals(e.modulo(this.ticksMajor._interval64))) {
                return true
            } else {
                return false
            }
        ...
        var f = this._getScaleLength(), e = this._getBorderSize(), h, j, i = this._getMaxTickSize(), g = i + e;
        if (this.int64 === true) {
            h = this._valueToCoordinates(this._max64);
            j = this._valueToCoordinates(this._min64)
        } else {
        ...
        var h, n, j, o, g = this._getScaleLength();
        k += this._getSize(this.labels.offset);
        if (this.int64 === true) {
            h = this.labels._interval64;
            n = (this._max64.subtract(this._min64)).div(h).toNumber();
            o = (this.orientation === "vertical") ? this._max64 : this._min64
        } else {
        ...
        for (var l = 0; l <= n; l += 1) {
            ...
            if (this.int64 === true) {
                o = (this.orientation === "vertical") ? o.subtract(h) : o.add(h)
            } else {
        ...
        k = this.labels.formatValue(n.toString(), m);
